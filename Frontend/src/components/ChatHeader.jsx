...
const ChatHeader = () => {
  ...
  return <div className="p-2 sm:p-2.5 border-b border-base-300">
    <div className="flex items-center justify-between">
      <div className="flex min-w-0 items-center gap-2 sm:gap-3">
        ...
        <div className="min-w-0"><div className="flex items-center gap-1"><h3 className="truncate font-medium">{selectedUser.nickname || selectedUser.fullName}</h3><button className="btn btn-ghost btn-xs btn-circle shrink-0" title="Set your nickname" onClick={() => { setNickname(selectedUser.nickname || ""); setEditingNickname(true); }}><Pencil size={13}/></button></div><p className="text-sm text-base-content/70">{onlineUsers.includes(selectedUser._id) ? "Online" : "Offline"}</p></div>
      </div>
      <div className="flex shrink-0 items-center gap-0 sm:gap-1"><label className="input input-sm hidden md:flex items-center gap-1"><Search size={14}/><input value={messageSearch} onChange={(e) => setMessageSearch(e.target.value)} placeholder="Search messages" /></label><button className="btn btn-ghost btn-sm btn-circle md:hidden" onClick={() => setShowSearch(!showSearch)} title="Search messages"><Search size={19}/></button>{!selectedUser.isBlocked && selectedUser.requestDirection === "received" && <button className="btn btn-success btn-xs" onClick={() => updateRelationship(selectedUser._id, "respond", true)}><Check size={14}/> <span className="hidden sm:inline">Accept</span></button>}{friends && <><button className="btn btn-ghost btn-sm btn-circle" title="Voice call" onClick={() => window.dispatchEvent(new CustomEvent("call:start", { detail: { user: selectedUser, video: false } }))}><Phone size={19}/></button><button className="btn btn-ghost btn-sm btn-circle" title="Video call" onClick={() => window.dispatchEvent(new CustomEvent("call:start", { detail: { user: selectedUser, video: true } }))}><Video size={19}/></button></>}<button className="hidden sm:flex btn btn-ghost btn-sm btn-circle" title={selectedUser.blockedByMe ? "Unblock user" : "Block user"} onClick={() => updateRelationship(selectedUser._id, "block")}><Ban size={18}/></button><button className="btn btn-ghost btn-sm btn-circle" onClick={() => setSelectedUser(null)}><X /></button></div>
    </div>
